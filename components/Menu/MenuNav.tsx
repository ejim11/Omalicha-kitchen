...
const MenuNav: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <MainContainer>
      ...
      >
        <div className="flex z-50 mt-[4rem] max-xmd:overflow-x-auto menuNav max-xmd:px-[1.5rem] max-xmd:flex-nowrap max-xmd:justify-between  justify-center relative text-[1.8rem] font-switzer">
          {navs.map((nav: string) => (
            <Link
              ...
              } block border-b-[0.4rem] pb-[2rem] mr-[4.8rem] bg-[#F4F3E8] w-max max-xmd:text-nowrap last:mr-0 transition-all duration-150 ease-in`}
            >
              {nav}
            </Link>
          ))}
